...
export default function BorrowerMessagesPage() {
  ...
  const loadThreads = async () => {
    try {
      ...
      // Get borrower ID through borrower_user_links
      ...
      setBorrowerId(linkData.borrower_id)

      // Get all message threads for this borrower
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !selectedThread || !borrowerId) return
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Communicate with lenders about your loan requests</p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {threads.filter(t => t.loan_offers?.status === 'pending').length}
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            {threads.length === 0 ? (
              <div className="text-center py-8 px-4">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Messages will appear when lenders make offers
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start space-x-3 flex-1">
                        <Avatar>
                          <AvatarFallback className="bg-primary text-white">
                            {thread.lenders?.business_name?.charAt(0) || 'L'}
                          </AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm truncate">
                            {thread.lenders?.business_name || 'Lender'}
                          </p>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="flex items-center space-x-2">
                      ...
                      <span>{selectedThread.lenders?.business_name || 'Lender'}</span>
                    </CardTitle>
                    ...
                    {selectedThread.loan_offers && (
                      <div className="space-y-1">
                        <p className="text-sm font-medium">
                          Offer: {formatCurrency(selectedThread.loan_offers.amount_minor)}
                        </p>
                        <p className="text-xs text-gray-600">
                          {selectedThread.loan_offers.apr_bps / 100}% APR • {selectedThread.loan_offers.term_months} months
                        </p>
                        <Badge className={
              ...
              <CardContent className="p-4">
                <div className="h-[400px] overflow-y-auto space-y-4 mb-4">
                  {messages.map((message) => {
                    const isBorrower = message.sender_type === 'borrower'
                    return (
                      <div
                        key={message.id}
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mt-1">
                            <p className={`text-xs ${isBorrower ? 'text-white/70' : 'text-gray-500'}`}>
                              {format(new Date(message.created_at), 'HH:mm')}
                            </p>
                            {isBorrower && message.read_at && (
