-- Fix generate_loan_agreement function to remove reference to non-existent provider_info table
-- The provider_info table does not exist, so we remove the LEFT JOIN and use business_name instead
...
BEGIN
  ...
  -- Get lender details (removed provider_info join as table doesn't exist)
  ...
  -- Get currency symbol from country_currency_allowed table (NOT countries table)
...
'Generates a professional loan agreement with clear interest breakdown. Shows Principal + Interest Rate + Interest Amount = Total, and payment type (once-off vs installments). Uses country_currency_allowed for currency symbol.';
