-- Fix process_repayment function to use correct column names
-- The loans table uses interest_amount_minor, not total_interest_minor
...
BEGIN
  ...
  -- Use interest_amount_minor (correct column name) instead of total_interest_minor
  SELECT
    ...
    COALESCE(l.interest_amount_minor, 0) as interest_amount_minor,
  ...
  ELSE
    -- No schedules, use loan principal + interest (using correct column name)
    v_total_due := (v_loan.principal_minor + v_loan.interest_amount_minor) / 100.0;
  END IF;
