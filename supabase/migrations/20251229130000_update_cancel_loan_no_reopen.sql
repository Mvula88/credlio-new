-- Update cancel_loan_by_borrower to NOT reopen the loan request
-- Borrower should create a new request instead of reusing the old one
...
BEGIN
  ...
  -- Mark the loan request as cancelled (NOT reopened)
  -- Borrower should create a new request if they want another loan
  ...
  INSERT INTO public.notifications (
    ...
    'The borrower has cancelled the loan before signing the agreement. The loan request has been closed.',
    'loan',
    p_loan_id,
    NOW()
  );
...
COMMENT ON FUNCTION public.cancel_loan_by_borrower(UUID) IS
  'Allows a borrower to cancel a loan ONLY if they have not yet signed the agreement.
   The loan request is also cancelled - borrower must create a new request for another loan.';
