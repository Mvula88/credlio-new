-- Fix accept_offer function to allow multiple active loans
-- Lenders should assess borrower risk (including existing loans) before making offers
-- The restriction should only be on OPEN loan requests, not on active loans
...
BEGIN
  ...
  -- NOTE: We removed the "one active loan" restriction
  -- Borrowers CAN have multiple active loans
  -- Lenders should assess the borrower's existing loans before making offers
  ...
  INSERT INTO public.loans (
    ...
    'pending_disbursement', -- Start as pending_disbursement until lender confirms sending money
    0,
    NOW()
  );
  ...
  -- Generate repayment schedule with all required columns
...
-- Also ensure the "one open request at a time" restriction exists
-- This is already handled in the frontend/RLS, but let's add a comment for clarity
COMMENT ON FUNCTION public.accept_offer(UUID) IS
  'Accepts a loan offer and creates a loan. Borrowers can have multiple active loans.
   The one-open-request-at-a-time restriction is handled at the request creation level.';
